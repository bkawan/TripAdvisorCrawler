...
class TripadvisorSpider(scrapy.Spider):
    ...
    def parse_attraction_contents(self, response):
        for select in response.xpath('//span[@class="format_address"]'):
            ...
            rank = sel.xpath("string(//div[1])").extract_first().strip()
            print(rank)
        ...
        #
        #
        #         # trip_item['rank'] = response.xpath('//span[@class=""]/text()').extract() + response.xpath(
        #         #     '//div[@class="slim_ranking"]/text()').extract() + response.xpath(
        #         #     '//div[@class="slim_ranking"]/a/text()').extract()
